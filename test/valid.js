...
describe('parseWithValidation', function () {
	...
	it('hillcrestpartyrentals.html  should return Object with valid: false.  not closing <p> tag on line 476', function () {
		const result = valid(fs.readFileSync(__dirname + '/html/hillcrestpartyrentals.html').toString());
		result.should.eql(false);
	})

	it('google.html  should return Object with valid: true', function () {
		const result = valid(fs.readFileSync(__dirname + '/html/google.html').toString());
		result.should.eql(true);
	})

	it('gmail.html  should return Object with valid: true', function () {
		const result = valid(fs.readFileSync(__dirname + '/html/gmail.html').toString());
		result.should.eql(true);
	})

	it('ffmpeg.html  should return Object with valid: false (extra opening <div>', function () {
		const result = valid(fs.readFileSync(__dirname + '/html/ffmpeg.html').toString());
		result.should.eql(false);
	})
	...
	it('gmail.html  should return Object with valid: true', function () {
		const result = valid(fs.readFileSync(__dirname + '/html/gmail.html').toString().replace(/<\//gi, '<'));
		result.should.eql(false);
	})

	it('gmail.html  should return Object with valid: true', function () {
		const result = valid(fs.readFileSync(__dirname + '/html/nice.html').toString().replace(/<\//gi, '<'));
		result.should.eql(false);
	})
});
